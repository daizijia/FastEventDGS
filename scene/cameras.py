...
from utils.image_utils import rgb2gray


class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", 
                 fid=None, depth=None, events=None, image_timestamp=None, closest_event_index=None, w = None, h = None):
        ...
        # new added part
        # self.intensity = None #rgb2gray(self.original_image) #bgr? [1,H,W]
        # self.events = None#[torch.Tensor(event).to(self.data_device) for event in events] if events is not None else None
        self.image_timestamp = image_timestamp
        self.closest_event_index = closest_event_index
        ...
        self.world_view_transform = torch.tensor(getWorld2View2(R, T, trans, scale)).transpose(0, 1).to(
            self.data_device) # extrinsic matrix
        ...
        # seems Pytorch3d axis in our case
