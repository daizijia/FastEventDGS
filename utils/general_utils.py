...
def cartesian_to_polar(patch):
    # patch: (H, W)
    device = patch.device
    h, w = patch.shape

    original_y, original_x = torch.nonzero(patch, as_tuple=True)

    center_x = torch.tensor((w - 1) / 2.0, device=device)
    center_y = torch.tensor((h - 1) / 2.0, device=device)

    x = original_x.float() - center_x
    y = center_y - original_y.float()

    distance = torch.hypot(x, y)
    theta = torch.atan2(y, x)

    return theta, distance, (original_y, original_x)
